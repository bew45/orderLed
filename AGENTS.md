...
- **Local PaddleOCR is FIXED and working on GPU** (2026-07-10). Default config now uses the shared Muse `paddlepaddle-gpu==3.3.0` venv (`C:\Users\newpo\.muse-manga-ocr-gpu`) with `paddle_device: "gpu"` — an RTX 3060 Ti. Real timing through the server worker path: ~6–7s for the first screenshot (worker cold start + model load), **~0.3s per screenshot once the worker is warm**. Verify with `npx tsx scripts/ocr-smoke.ts` — it runs the real settings → worker → queue → amount-scan path against the latest uploaded screenshots.
  - **Root cause #1 (fixed by config):** settings/`.env` originally pointed `paddle_python` at that same GPU venv but `paddle_device` defaulted to `"gpu"` while the venv's `paddlepaddle-gpu 3.3.0` failed to load `cublasLt64_13.dll` — see root cause #2. A short-lived earlier fix pinned everything to the project's own `.venv-ocr` (`paddlepaddle==3.2.2`, CPU-only, ~8–13s/screenshot) as a working fallback; that venv and pin are still kept as a backup path (`requirements-ocr.txt`, do not delete) but are no longer the default.
  - **Root cause #2 (the real GPU bug, fixed 2026-07-10):** `paddlepaddle-gpu 3.3.0`'s Windows build could not locate `cublasLt64_13.dll` at runtime even with the file present and independently loadable via `ctypes.WinDLL(...)` — Paddle's own dynamic loader does not honor `PATH`/`os.add_dll_directory()`/`CUDA_PATH` reliably on this build. **Fix:** hardlink every CUDA/cuDNN DLL from `Lib\site-packages\nvidia\cu13\bin\x86_64\*.dll` and `Lib\site-packages\nvidia\cudnn\bin\*.dll` directly into `Lib\site-packages\paddle\libs\` inside that venv, so they sit in the same directory Windows always searches first (the directory of the DLL doing the loading — `paddle\libs\phi.dll`/`common.dll`). This is a **venv-local filesystem change**, not something `requirements-ocr.txt`/pip can express — if that venv is ever reinstalled/recreated, redo the hardlink step (PowerShell `New-Item -ItemType HardLink`) before expecting GPU to work again. It's additive-only and safe for Muse's own manga OCR that shares this venv.
  - The worker still auto-falls back GPU→CPU at init if GPU init ever fails again (`scripts/paddle_ocr_worker.py`), and the runner drains the queue after 3 consecutive init failures instead of hanging every screenshot (`server/ocr/ocr-runner.ts`).
  - If OCR ever fails outright, extraction still works: `processBatch` catches the OCR error and continues to OpenRouter; only the amount-check verifier degrades to `"unavailable"` (rows land in `needs_check`).
  - **Do not upgrade `paddlepaddle-gpu` in the Muse venv** without retesting — a version bump may ship DLLs to a different path and silently undo the hardlink fix.
